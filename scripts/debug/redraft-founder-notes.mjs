...
(async () => {
  ...
  for (const email of EMAILS) {
    ...
    if (!CONFIRM) { log(`${email}: would trash gmail draft ${oldDraftId || '(none)'}, delete row ${row?.id || '(none)'}, redraft with cta=${cta}`); continue; }
    ...
    const r = await fetch(FN_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ max: 1, only: email, cta }) });
    log(`${email}: queue POST → ${r.status} (cta=${cta}); waiting for the new draft…`);
