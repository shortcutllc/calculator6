...
| **3B** — Parity-critical batch (both viewers): custom line items + gratuity admin picker + office-address-per-location editor + pricing-options inline editor + auto-recurring discount picker + day-level summary in client view + Stripe invoice V2 restyle + pro-level numbers editable + admin internal financials in pricing summary | ✅ shipped | `ProposalViewerV2.tsx`, `StandaloneProposalViewerV2.tsx`, `PricingOptionsSelector.tsx`, `ServiceCard.tsx`, `DaySummaryBox.tsx` |
| **3C** — Client UX completeness: approve confirm modal, help modal, view-original toggle, large multi-option grid, cost-per-headshot, client notes textarea, full client edit mode (params + pricing options) | ✅ shipped | `StandaloneProposalViewerV2.tsx`, `ApproveConfirmModal.tsx`, `HelpModal.tsx`, `OptionsTabs.tsx`, `ServiceCard.tsx` |
| **3D** — Admin polish + net-new: account-team override, force-approve / mark-rejected, view-as-client, mark-reviewed in change history, save-notes flow, collapsible sections, ServiceAgreement preview in admin, bulk service-type swap, test-send, recurring partner badge, InvoiceConfirmationModalV2 (full V2 restyle) | ✅ shipped | `ProposalViewerV2.tsx`, `InvoiceConfirmationModalV2.tsx`, `ServiceCard.tsx` |
| **4** — *Originally* "Modals: ChangeConfirmationModal restyle, ApproveConfirmModal, ChangeHistoryDrawer". | **collapsed into 3A/3C/3D** | RequestChangesModal shipped in 2D; ApproveConfirmModal in 3C; ChangeHistoryDrawer baseline in 3A + mark-reviewed actions in 3D. The originally-speculative "notification-prefs modal" was never on the V1 parity list — defer indefinitely unless requested. |
| **5** — Mindfulness-only proposal sections (Why Shortcut / Participant Benefits / CLE outline / Resources / Facilitator card / 5-slug backfill) | not started | `MindfulnessProposalContent.tsx` restyle + new mindfulness section components |
| **6** — Gallery backend + Trust real logos + `recurringFrequency` data migration | not started | New `proposal_gallery` table + storage bucket |
