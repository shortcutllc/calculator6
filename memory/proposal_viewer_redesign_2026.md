...
- [ ] **Auto-recurring discount picker (admin)** — No / 10% / 15% / 20% + per-service savings callout. **Needs product decision first**: keep auto-recurring as a separate data flag (V1 model: `data.isAutoRecurring` + `data.autoRecurringDiscount`) or migrate fully to V2's frequency × volume-discount model. Ping Will before implementing.
